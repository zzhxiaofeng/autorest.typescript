...
try {
    ...
    $nodeModulesFolders = Get-ChildItem -Path "$root" -Filter "node_modules" -Recurse -Directory
    $nodeModulesFolders | Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
    ...
    if ($BuildArtifactsPath) {
        ...
        Invoke-LoggedCommand "node common/scripts/install-run-rush.js install"
      }
